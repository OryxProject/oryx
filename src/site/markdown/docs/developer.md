...
... where `x.y.z` is the current version string, like `2.0.1`.
...
The application JAR is produced at `target/example-2.0.1.jar` for example.
...
```
./oryx-run.sh batch --conf wordcount-example.conf --app-jar example-2.0.1.jar
```
